...
**面向 Profiles、Hosts 和终端工作流的会话管理工具**
...
- 对在线会话执行 Attach；对已保存但当前离线的会话执行 Restore & Attach
- 根据当前平台检测并调用可用终端
...
3. 在 `Sessions` 页面用 `Shell`、`SSH Host` 或 `Command` 模式创建会话。
4. 在线会话使用 `Attach`，离线但已保存的会话使用 `Restore & Attach`。

Mole 会把 session 元数据和真实运行时 backend 分开存储。所以即使 backend 进程暂时没了，保存过的 session 依然会留在 UI 中，并且可以通过 `Restore & Attach` 按原配置重建。
