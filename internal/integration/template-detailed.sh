#!/bin/bash
# Mole detailed template for xbar/SwiftBar — full display, no truncation
...
# Dens: show all burrows for each den (no truncation)
if den_sessions:
...
# Burrows: show all no-den sessions (no truncation)
if no_den:
