...
// Store handles launch plugin preset persistence.
type Store struct {
	path string
	mu   sync.RWMutex
}
...
func (s *Store) load() ([]Config, error) {
	...
	for i := range configs {
		if configs[i].Settings == nil {
			configs[i].Settings = map[string]string{}
		}
	}
	return configs, nil
}
...
func (s *Store) ReplaceAll(configs []Config) error {
	s.mu.Lock()
	defer s.mu.Unlock()
